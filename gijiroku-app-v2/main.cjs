  ...
  "node_modules/dotenv/lib/main.js"(exports2, module2) {
    ...
    function configDotenv(options) {
      ...
      for (const path13 of optionPaths) {
        try {
          const parsed = DotenvModule.parse(fs10.readFileSync(path13, { encoding }));
    ...
    FormData3.prototype._lengthRetriever = function(value, callback) {
      if (hasOwn(value, "fd")) {
        if (value.end != void 0 && value.end != Infinity && value.start != void 0) {
          ...
          fs10.stat(value.path, function(err, stat4) {
            if (err) {
              callback(err);
              return;
            }
...
var WindowManager = class {
  mainWindow = null;
  isDevelopment = !import_electron.app.isPackaged;
  // より確実な環境判定
  async createMainWindow() {
    console.log("\u{1FA9F} Creating main window...");
    console.log("\u{1F4F1} Environment:", {
      isDevelopment: this.isDevelopment,
      isPackaged: import_electron.app.isPackaged,
      nodeEnv: process.env.NODE_ENV
    });
    console.log("\u{1F4C1} App paths:", {
      appPath: import_electron.app.getAppPath(),
      userData: import_electron.app.getPath("userData"),
      exe: import_electron.app.getPath("exe"),
      cwd: process.cwd(),
      __dirname
    });
    ...
    if (process.env.DISABLE_GPU === "true") {
      ...
      console.log("\u{1F6AB} Hardware acceleration disabled");
    }
    const getIconPath = () => {
    ...
    console.log("\u{1F3A8} Icon path:", iconPath);
    const windowConfig = {
      ...
      webPreferences: {
        ...
        webSecurity: true,
        // 本番環境では常にセキュリティを有効化
        preload: import_electron.app.isPackaged ? path.join(__dirname, "preload.cjs") : path.join(__dirname, "preload.cjs")
      },
      titleBarStyle: "default"
    };
    console.log("Window config:", windowConfig);
    this.mainWindow = new import_electron.BrowserWindow(windowConfig);
  ...
  async loadDevelopment() {
    ...
    try {
      console.log(`\u{1F50C} Connecting to dev server: ${devServerUrl}`);
  ...
  async loadProduction() {
    if (!this.mainWindow) return;
    const indexPath = import_electron.app.isPackaged ? path.join(__dirname, "../dist/index.html") : path.join(__dirname, "../../dist/index.html");
    console.log("\u{1F4C4} Loading production HTML from:", indexPath);
  ...
  setupErrorHandlers() {
    ...
    this.mainWindow.webContents.on("did-fail-load", (event, errorCode, errorDesc, validatedURL) => {
      console.error("\u{1F6A8} did-fail-load:", { errorCode, errorDesc, validatedURL });
    });
    this.mainWindow.webContents.on("render-process-gone", (event, details) => {
      console.error("\u{1F6A8} render-process-gone:", details);
      if (details.reason === "crashed") {
        this.showCrashDialog();
      }
    });
    this.mainWindow.webContents.on("unresponsive", () => {
      console.error("\u{1F6A8} Renderer process became unresponsive");
    });
    ...
    this.mainWindow.webContents.on("console-message", (event, level, message, line, sourceId) => {
      ...
      console[logLevel](`[Renderer ${level}] ${message} (${sourceId}:${line})`);
    });
  }
  showCrashDialog() {
    console.log("\u{1F4A5} Renderer crashed - showing recovery options");
  }
  getErrorHtml() {
    return `
      <!DOCTYPE html>
      <html>
        ...
        <body style="font-family: system-ui; padding: 40px; background: #f5f5f5; color: #333;">
          ...
          <p><small>\u30D1\u30C3\u30B1\u30FC\u30B8\u7248: ${import_electron.app.isPackaged}</small></p>
        </body>
      </html>
    `;
  }
  ...
  setupWindowEvents() {
    ...
    this.mainWindow.webContents.on("will-navigate", (event, url2) => {
      if (!this.isDevelopment && !url2.startsWith("file://")) {
        event.preventDefault();
        console.warn("\u{1F6AB} Blocked navigation to:", url2);
      }
    });
  }
  getMainWindow() {
...
// app/src/main/services/WorkspaceService.ts
init_cjs_shims();
...
var WorkspaceConfigSchema = external_exports.object({
  ...
  paths: external_exports.object({
    attachments: external_exports.string(),
    exports: external_exports.string(),
    cache: external_exports.string(),
    db: external_exports.string()
  })
});
var AppLocalConfigSchema = external_exports.object({
  pdf: external_exports.object({
    pageSize: external_exports.enum(["A4", "Letter"]).default("A4"),
    marginMm: external_exports.number().default(15)
  }),
  markdown: external_exports.object({
    theme: external_exports.enum(["default", "corporate"]).default("default"),
    toc: external_exports.boolean().default(true),
    latex: external_exports.enum(["katex", "tectonic"]).default("katex")
  }),
  mermaid: external_exports.object({
    themePath: external_exports.string().default("../../resources/themes/mermaid/mermaid-theme.css")
  })
});
var WorkspaceService = class _WorkspaceService {
  ...
  async resolve() {
    if (this.workspaceRoot) {
      const info2 = await this.getWorkspaceInfo(this.workspaceRoot);
      return {
        root: this.workspaceRoot,
        paths: this.resolvePaths(this.workspaceRoot, info2.config.paths)
      };
    }
    const envWorkspace = process.env.GIJIROKU_WORKSPACE_DIR;
    if (envWorkspace) {
      try {
        const resolved = path2.resolve(envWorkspace);
        if (await this.isValidWorkspace(resolved)) {
          this.workspaceRoot = resolved;
          const info2 = await this.getWorkspaceInfo(resolved);
          return {
            root: resolved,
            paths: this.resolvePaths(resolved, info2.config.paths)
          };
        }
      } catch (error) {
    ...
    const portableWorkspace = path2.join(path2.dirname(execPath), "workspace");
    try {
      if (await this.isValidWorkspace(portableWorkspace)) {
        this.workspaceRoot = portableWorkspace;
        const info2 = await this.getWorkspaceInfo(portableWorkspace);
        return {
          root: portableWorkspace,
          paths: this.resolvePaths(portableWorkspace, info2.config.paths)
        };
      }
    } catch (error) {
      console.warn("\u30DD\u30FC\u30BF\u30D6\u30EB\u30EF\u30FC\u30AF\u30B9\u30DA\u30FC\u30B9\u304C\u7121\u52B9:", error);
    }
    const userDataWorkspace = path2.join(import_electron2.app.getPath("userData"), "workspace");
  ...
  async initializeWorkspace(workspaceRoot) {
    ...
    for (const dir of dirs) {
      await fs2.mkdir(path2.join(workspaceRoot, dir), { recursive: true });
    }
   ...
   * workspace.json を作成
   */
  async createWorkspaceConfig(workspaceRoot) {
    const config2 = {
    ...
    const configPath = path2.join(workspaceRoot, "config", "workspace.json");
    await fs2.writeFile(configPath, JSON.stringify(config2, null, 2), "utf8");
  }
  /**
   * app.local.json を作成
   */
  async createAppLocalConfig(workspaceRoot) {
    const config2 = {
    ...
    const configPath = path2.join(workspaceRoot, "config", "app.local.json");
    await fs2.writeFile(configPath, JSON.stringify(config2, null, 2), "utf8");
  }
  /**
   * resources/templates/ → workspace/templates/ をコピー
   */
  async copyResourceTemplates(workspaceRoot) {
    const resourcesDir = this.getResourcesDir();
    const sourceTemplatesDir = path2.join(resourcesDir, "templates");
    const targetTemplatesDir = path2.join(workspaceRoot, "templates");
   ...
   * 必要に応じてテンプレートを更新
   */
  async updateTemplatesIfNeeded(workspaceRoot) {
    const resourcesDir = this.getResourcesDir();
    const sourceTemplatesDir = path2.join(resourcesDir, "templates");
    const targetTemplatesDir = path2.join(workspaceRoot, "templates");
    if (!await this.pathExists(sourceTemplatesDir)) {
    ...
    try {
      const sourceFiles = await fs2.readdir(sourceTemplatesDir, { recursive: true });
      const targetFiles = await fs2.readdir(targetTemplatesDir, { recursive: true });
   ...
   * ワークスペースが有効かチェック
   */
  async isValidWorkspace(workspaceRoot) {
    try {
      const configPath = path2.join(workspaceRoot, "config", "workspace.json");
  ...
  async getWorkspaceInfo(workspaceRoot) {
    ...
    const configPath = path2.join(workspaceRoot, "config", "workspace.json");
    const appConfigPath = path2.join(workspaceRoot, "config", "app.local.json");
    const configData = await fs2.readFile(configPath, "utf8");
    const appConfigData = await fs2.readFile(appConfigPath, "utf8");
    const config2 = WorkspaceConfigSchema.parse(JSON.parse(configData));
    const appConfig = AppLocalConfigSchema.parse(JSON.parse(appConfigData));
    this.workspaceInfo = {
      root: workspaceRoot,
      paths: this.resolvePaths(workspaceRoot, config2.paths),
      config: config2,
      appConfig
    };
    return this.workspaceInfo;
   ...
   * 相対パスを絶対パスに解決
   */
  resolvePaths(workspaceRoot, paths) {
    const resolved = {};
    for (const [key, relativePath] of Object.entries(paths)) {
      resolved[key] = path2.resolve(workspaceRoot, relativePath);
    }
    return resolved;
  }
  /**
  ...
  getResourcesDir() {
    if (import_electron2.app.isPackaged) {
      return path2.join(process.resourcesPath, "resources");
    } else {
      return path2.join(__dirname, "../../resources");
    }
  }
  /**
   * ディレクトリをコピー（再帰的）
  ...
  async copyDirectory(source, target) {
    ...
    const entries = await fs2.readdir(source, { withFileTypes: true });
  ...
  async updateAppConfig(updates) {
    ...
    const configPath = path2.join(root, "config", "app.local.json");
    await fs2.writeFile(configPath, JSON.stringify(newConfig, null, 2), "utf8");
...
// app/src/main/services/DbService.ts
init_cjs_shims();
...
var UserSchema = external_exports.object({
  ...
  external_id: external_exports.string().nullable().optional(),
...
var MeetingSchema = external_exports.object({
  id: external_exports.string().uuid(),
  owner_id: external_exports.string().uuid(),
  title: external_exports.string(),
  started_at: external_exports.string().nullable().optional(),
  ended_at: external_exports.string().nullable().optional(),
  location: external_exports.string().nullable().optional(),
  status: external_exports.enum(["draft", "processing", "done", "archived"]).default("draft"),
  created_at: external_exports.string(),
  updated_at: external_exports.string(),
...
var DocumentSchema = external_exports.object({
  id: external_exports.string().uuid(),
  meeting_id: external_exports.string().uuid(),
  type: external_exports.enum(["minutes", "summary", "todo", "markdown", "pdf_meta"]),
  title: external_exports.string().nullable().optional(),
  current_version_id: external_exports.string().uuid().nullable().optional(),
  created_at: external_exports.string(),
  updated_at: external_exports.string()
});
var DocumentVersionSchema = external_exports.object({
  id: external_exports.string().uuid(),
  document_id: external_exports.string().uuid(),
  content_md: external_exports.string().nullable().optional(),
...
var TaskSchema = external_exports.object({
  ...
  status: external_exports.enum(["open", "in_progress", "blocked", "done"]).default("open"),
  source_segment_id: external_exports.number().nullable().optional(),
  created_at: external_exports.string(),
  updated_at: external_exports.string()
});
var AttachmentSchema = external_exports.object({
  id: external_exports.string().uuid(),
  meeting_id: external_exports.string().uuid(),
  type: external_exports.enum(["source_vtt", "audio", "pdf", "image", "other"]),
  storage_path: external_exports.string(),
  content_hash: external_exports.string(),
  mime: external_exports.string().nullable().optional(),
  bytes: external_exports.number().nullable().optional(),
  created_at: external_exports.string()
});
var JobSchema = external_exports.object({
  ...
  // JSON string
  status: external_exports.enum(["queued", "running", "succeeded", "failed", "canceled"]).default("queued"),
  progress: external_exports.number().default(0),
...
var DbService = class _DbService {
  ...
  async initialize() {
    ...
    const workspaceService2 = WorkspaceService.getInstance();
    const { paths } = await workspaceService2.resolve();
    const dbPath = paths.db;
    console.log("SQLite\u30C7\u30FC\u30BF\u30D9\u30FC\u30B9\u521D\u671F\u5316:", dbPath);
    const dbDir = path3.dirname(dbPath);
    await import("fs/promises").then((fs10) => fs10.mkdir(dbDir, { recursive: true }));
  ...
  getStats() {
    ...
    const meetingCount = db.prepare("SELECT COUNT(*) as count FROM meetings WHERE deleted_at IS NULL").get();
    const documentCount = db.prepare("SELECT COUNT(*) as count FROM documents").get();
    const jobStats = db.prepare("SELECT status, COUNT(*) as count FROM jobs GROUP BY status").all();
   ...
   * 古いジョブを削除（クリーンアップ用）
   */
  cleanupOldJobs(olderThanDays = 30) {
    try {
      const db = this.getDb();
      ...
      if (result.changes > 0) {
        console.log(`\u{1F9F9} Cleaned up ${result.changes} old jobs`);
      }
   ...
   * 失敗したジョブを再キュー（管理用）
   */
  requeueFailedJobs(maxRetries = 3) {
    try {
      const db = this.getDb();
      const now = (/* @__PURE__ */ new Date()).toISOString();
      const stmt = db.prepare(`
      ...
      if (result.changes > 0) {
        console.log(`\u{1F504} Requeued ${result.changes} failed jobs`);
      }
      return result.changes;
    } catch (error) {
...
// app/src/main/services/MarkdownCompilerService.ts
init_cjs_shims();
...
// app/src/main/services/MermaidRenderWorker.ts
init_cjs_shims();
var import_electron3 = require("electron");
var crypto = __toESM(require("crypto"), 1);
var path4 = __toESM(require("path"), 1);
var fs3 = __toESM(require("fs/promises"), 1);
var MermaidRenderWorker = class _MermaidRenderWorker {
  static instance;
  workspaceService;
  renderWindow = null;
  isInitialized = false;
  ...
  static getInstance() {
    if (!_MermaidRenderWorker.instance) {
      _MermaidRenderWorker.instance = new _MermaidRenderWorker();
    }
    return _MermaidRenderWorker.instance;
  }
  /**
  ...
  async initialize() {
    if (this.isInitialized) return;
    console.log("\u{1F9E9} Initializing MermaidRenderWorker...");
    try {
      console.log("\u{1FA9F} Creating render window...");
      this.renderWindow = new import_electron3.BrowserWindow({
        show: false,
        // hidden window
        webPreferences: {
          nodeIntegration: false,
      ...
      console.log("\u{1F4C4} Loading Mermaid HTML template...");
      ...
      await Promise.race([
        ...
        new Promise(
          (_, reject) => setTimeout(() => reject(new Error("MermaidWorker page load timeout")), 15e3)
        )
      ]);
      this.isInitialized = true;
   ...
   */
  async renderToSvg(mermaidCode, options = {}) {
   ...
   * 実際のMermaidレンダリングを実行
   */
  async executeMermaidRendering(mermaidCode, options) {
   ...
   * キャッシュされたSVGを取得
   */
  async getCachedSvg(hash) {
    try {
      const { paths } = await this.workspaceService.resolve();
      const cachePath = path4.join(paths.cache, "mermaid", `${hash}.svg`);
      const svg = await fs3.readFile(cachePath, "utf-8");
   ...
   * SVGをキャッシュに保存
   */
  async saveSvgToCache(hash, svg) {
    try {
      const { paths } = await this.workspaceService.resolve();
      const cacheDir = path4.join(paths.cache, "mermaid");
      const cachePath = path4.join(cacheDir, `${hash}.svg`);
      await fs3.mkdir(cacheDir, { recursive: true });
      await fs3.writeFile(cachePath, svg, "utf-8");
   ...
   * ハッシュ値を生成（キャッシュキー用）
   */
  generateHash(mermaidCode, options) {
...
<body>
      ...
      try {
        ...
        for (const file of files) {
          if (file.endsWith(".svg")) {
            await fs3.unlink(path4.join(cacheDir, file));
          }
        }
   ...
   */
  isReady() {
    return this.isInitialized && this.renderWindow !== null && !this.renderWindow.isDestroyed();
  }
};
...
var FrontMatterSchema = external_exports.object({
  ...
  pageSize: external_exports.enum(["A4", "Letter"]).default("A4"),
  marginMm: external_exports.number().default(15),
  format: external_exports.enum(["standard", "latex"]).default("standard"),
  mermaid: external_exports.object({
    theme: external_exports.string().default("default")
  }).optional()
});
var MarkdownCompilerService = class _MarkdownCompilerService {
  static instance;
  workspaceService;
  mermaidWorker;
  processor;
  ...
  static getInstance() {
    if (!_MarkdownCompilerService.instance) {
      _MarkdownCompilerService.instance = new _MarkdownCompilerService();
    }
    return _MarkdownCompilerService.instance;
  }
  /**
  ...
  async initializeProcessor() {
    try {
      console.log("\u{1F504} Initializing remark processor...");
      this.processor = (0, import_remark.remark)();
      try {
      ...
      try {
        ...
        this.processor = this.processor.use(import_remark_rehype.default, {
          allowDangerousHtml: true
        });
      }
      try {
        ...
        try {
          this.processor = this.processor.use(import_rehype_raw.default);
        } catch (fallbackError) {
      ...
      try {
        ...
        try {
          this.processor = this.processor.use(import_rehype_stringify.default);
        } catch (fallbackError) {
   ...
   */
  async compileToHtml(input) {
    await this.ensureInitialized();
    const warnings = [];
    try {
      let mdContent;
      if (input.mdPath) {
        mdContent = await fs4.readFile(input.mdPath, "utf-8");
      ...
      if (input.options?.includeImages && input.options?.imageData) {
        console.log("\u{1F5BC}\uFE0F  Processing images for PDF generation...");
        mdContent = await this.preprocessImages(mdContent, input.options.imageData, warnings);
      }
      const matter = (0, import_gray_matter.default)(mdContent);
   ...
   */
  async preprocessImages(content, imageData, warnings) {
    ...
    try {
      Object.entries(imageData).forEach(([imageId, dataUri]) => {
        ...
        if (matches) {
          console.log(`\u{1F5BC}\uFE0F  Replacing image ID "${imageId}" with base64 data (${matches.length} occurrences)`);
   ...
   */
  async preprocessMath(content, format, warnings) {
    if (format !== "latex") {
      return content;
    }
    try {
      console.log("\u{1F9EE} Processing LaTeX math expressions...");
      let processedContent = content;
   ...
   * Mermaidコードブロックを事前描画
   */
  async preprocessMermaid(content, warnings) {
   ...
   * カスタムスタイルを適用
   */
  async applyCustomStyles(htmlContent, frontMatter) {
    try {
      const { paths } = await this.workspaceService.resolve();
      ...
      console.log("\u{1F527} Template processing:");
      console.log("  - Title:", frontMatter.title || "Document");
      console.log("  - Template contains {{TITLE}}:", templateContent.includes("{{TITLE}}"));
      const finalHtml = templateContent.replace(/\{\{TITLE\}\}/g, frontMatter.title || "Document").replace(/\{\{AUTHOR\}\}/g, frontMatter.author || "").replace(/\{\{DATE\}\}/g, frontMatter.date || (/* @__PURE__ */ new Date()).toLocaleDateString("ja-JP")).replace(/\{\{KATEX_CSS\}\}/g, katexCss).replace(/\{\{MERMAID_CSS\}\}/g, mermaidCss).replace(/\{\{CUSTOM_CSS\}\}/g, customCss).replace(/\{\{CONTENT\}\}/g, htmlContent);
   ...
   * HTMLテンプレートを読み込み
   */
  async loadHtmlTemplate(theme) {
    const resourcesDir = this.getResourcesDir();
    const templatePath = path5.join(resourcesDir, "templates", "html", `${theme}.html`);
    try {
      return await fs4.readFile(templatePath, "utf-8");
    } catch (error) {
...
<body>
  ...
  async getKatexCss() {
    const resourcesDir = this.getResourcesDir();
    const katexCssPath = path5.join(resourcesDir, "katex", "katex.min.css");
    try {
      return await fs4.readFile(katexCssPath, "utf-8");
    } catch (error) {
  ...
  async getMermaidCss() {
    const resourcesDir = this.getResourcesDir();
    const mermaidCssPath = path5.join(resourcesDir, "themes", "mermaid", "mermaid-theme.css");
    try {
      return await fs4.readFile(mermaidCssPath, "utf-8");
    } catch (error) {
...
`;
  ...
  getResourcesDir() {
    if (process.env.NODE_ENV === "development") {
      return path5.join(__dirname, "../../../resources");
    } else {
      return path5.join(process.resourcesPath, "resources");
    }
  }
};

// app/src/main/services/PdfGenerationService.ts
init_cjs_shims();
var import_electron4 = require("electron");
var path6 = __toESM(require("path"), 1);
var fs5 = __toESM(require("fs/promises"), 1);
var PdfGenerationService = class _PdfGenerationService {
  static instance;
  workspaceService;
  dbService;
  markdownCompiler;
  ...
  static getInstance() {
    if (!_PdfGenerationService.instance) {
      _PdfGenerationService.instance = new _PdfGenerationService();
    }
    return _PdfGenerationService.instance;
  }
  /**
   * MarkdownからPDFを生成
   */
  async generatePdfFromMarkdown(input, options = {}) {
    try {
      console.log("\u{1F680} Starting PDF generation...");
      console.log("\u{1F4C4} Input:", JSON.stringify(input).substring(0, 200) + "...");
      console.log("\u{1F9E9} Skipping MermaidWorker initialization (disabled for PDF generation)");
      console.log("\u{1F4DD} Compiling Markdown to HTML...");
      console.log("\u{1F4DD} Markdown content length:", input.mdContent?.length || 0);
      const compileResult = await this.markdownCompiler.compileToHtml(input);
      console.log("\u2705 HTML compilation completed");
      console.log("\u{1F4C4} HTML length:", compileResult.htmlContent?.length || 0);
      const pdfOptions = this.preparePdfOptions(compileResult.frontMatter, options);
      const pdfWindow = await this.createPdfWindow();
      try {
        console.log("\u{1F310} Loading HTML content...");
        console.log("\u{1F4DD} HTML content length:", compileResult.htmlContent?.length || 0);
        const encodedHtml = encodeURIComponent(compileResult.htmlContent);
        const dataUrl = `data:text/html;charset=utf-8,${encodedHtml}`;
        console.log("\u{1F4DD} Loading via data URL...");
        await pdfWindow.webContents.loadURL(dataUrl);
        console.log("\u2705 HTML content loaded");
        console.log("\u23F1\uFE0F Waiting for rendering...");
        await new Promise((resolve3) => setTimeout(resolve3, 2e3));
        console.log("\u2705 Rendering completed");
        console.log("\u{1F5A8}\uFE0F Generating PDF with printToPDF...");
        const pdfBuffer = await Promise.race([
          pdfWindow.webContents.printToPDF(pdfOptions),
          new Promise(
            (_, reject) => setTimeout(() => reject(new Error("PDF generation timeout")), 6e4)
          )
        ]);
        console.log("\u2705 PDF generated successfully", pdfBuffer.length, "bytes");
        const titleForFile = input.options?.title || compileResult.frontMatter.title || "document";
        console.log("\u{1F4C2} PDF title for filename:", titleForFile);
        console.log("\u{1F4C2} Input options title:", input.options?.title);
        console.log("\u{1F4C2} FrontMatter title:", compileResult.frontMatter.title);
        ...
        console.log("\u{1F9F9} Cleaning up PDF window...");
      ...
      const errorMessage = error instanceof Error ? `PDF generation failed: ${error.name} - ${error.message}` : `PDF generation failed: ${error}`;
   ...
   */
  async createPdfWindow() {
    try {
      console.log("\u{1F310} Creating PDF generation window...");
      ...
      window2.webContents.on("did-fail-load", (event, errorCode, errorDescription, validatedURL) => {
      ...
      window2.webContents.on("crashed", (event, killed) => {
   ...
   * printToPDFオプションを準備
   */
  preparePdfOptions(frontMatter, options) {
  ...
  async waitForPageLoad(window2) {
    return new Promise((resolve3, reject) => {
      ...
      window2.webContents.once("did-finish-load", resolveOnce);
      window2.webContents.once("dom-ready", resolveOnce);
      window2.webContents.once("did-fail-load", (event, errorCode, errorDescription) => {
        if (!hasResolved) {
          hasResolved = true;
          clearTimeout(timeout);
   ...
   * PDFファイルを保存
   */
  async savePdfFile(pdfBuffer, input, title) {
    const { paths } = await this.workspaceService.resolve();
    let fileName;
    if (input.pdfPath) {
    ...
    const pdfPath = path6.join(paths.exports, fileName);
    await fs5.mkdir(path6.dirname(pdfPath), { recursive: true });
    await fs5.writeFile(pdfPath, pdfBuffer);
    return pdfPath;
  }
  /**
   * PDFのページ数を推定（簡易版）
  ...
  generateDocumentId(input) {
    if (input.mdPath) {
      return path6.basename(input.mdPath, ".md");
    } else if (input.mdContent) {
      const crypto3 = require("crypto");
      return crypto3.createHash("sha256").update(input.mdContent).digest("hex").substring(0, 8);
    }
    return "unknown";
  }
  /**
...
// app/src/main/services/SecureStorageService.ts
init_cjs_shims();
var keytar = __toESM(require("keytar"), 1);
var SERVICE_NAME = "gijiroku-app-v2";
var ACCOUNT_PREFIX = "user-";
var SecureCredentialSchema = external_exports.object({
  id: external_exports.string().min(1),
  type: external_exports.enum(["supabase_key", "api_token", "oauth_token", "user_credential"]),
  service: external_exports.string().min(1),
  // 'supabase', 'openai', 'google', etc.
  account: external_exports.string().min(1),
  encrypted: external_exports.boolean().default(true),
  metadata: external_exports.record(external_exports.any()).optional()
});
var ApiKeyConfigSchema = external_exports.object({
  supabase: external_exports.object({
    url: external_exports.string().url().optional(),
    anonKey: external_exports.string().optional(),
    serviceRoleKey: external_exports.string().optional()
  }).optional(),
  openai: external_exports.object({
    apiKey: external_exports.string().optional(),
    organization: external_exports.string().optional()
  }).optional(),
  external: external_exports.record(external_exports.object({
    key: external_exports.string(),
    endpoint: external_exports.string().url().optional()
  })).optional()
});
var SecureStorageService = class _SecureStorageService {
  static instance;
  workspaceService;
  dbService;
  initialized = false;
  ...
  static getInstance() {
    if (!_SecureStorageService.instance) {
      _SecureStorageService.instance = new _SecureStorageService();
    }
    return _SecureStorageService.instance;
  }
  /**
   * サービス初期化
   */
  async initialize() {
    ...
    console.log("\u{1F510} Initializing SecureStorageService...");
    try {
   ...
   * 機密情報を安全に保存
   */
  async setCredential(id, type, service, account, value, options = {}) {
    await this.initialize();
    try {
      const credential = {
        id,
      ...
      try {
        ...
        console.log(`\u{1F510} Credential stored securely: ${id}`);
      } catch (keytarError) {
   ...
   * 機密情報を安全に取得
   */
  async getCredential(id, account) {
    await this.initialize();
    try {
      const keystoreAccount = `${ACCOUNT_PREFIX}${account}-${id}`;
      try {
   ...
   * 機密情報を削除
   */
  async deleteCredential(id, account) {
    await this.initialize();
    try {
      const keystoreAccount = `${ACCOUNT_PREFIX}${account}-${id}`;
      let deleted = false;
  ...
  async setApiConfig(config2) {
    ...
    try {
      if (config2.supabase) {
        if (config2.supabase.anonKey) {
          await this.setCredential(
            "supabase-anon-key",
            "supabase_key",
            "supabase",
            "anon",
            config2.supabase.anonKey
          );
        }
        if (config2.supabase.serviceRoleKey) {
          await this.setCredential(
            "supabase-service-key",
            "supabase_key",
            "supabase",
            "service",
            config2.supabase.serviceRoleKey
          );
        }
      }
      ...
      if (config2.external) {
        for (const [serviceName, serviceConfig] of Object.entries(config2.external)) {
          await this.setCredential(
            `${serviceName}-api-key`,
            "api_token",
            serviceName,
  ...
  async healthCheck() {
    try {
      ...
      return {
        ...
        keytar: keytarOk,
        fallback: fallbackOk,
        message: ok ? "Secure storage is operational" : "Secure storage is not available"
      };
    } catch (error) {
      return {
        ok: false,
   ...
   */
  async setCredentialFallback(id, value, credential) {
    const { paths } = await this.workspaceService.resolve();
   ...
   * フォールバック: ワークスペースから取得
   */
  async getCredentialFallback(id) {
    const { paths } = await this.workspaceService.resolve();
    const credentialsPath = require("path").join(paths.cache, "credentials.json");
    try {
   ...
   * フォールバック: ワークスペースから削除
   */
  async deleteCredentialFallback(id) {
    const { paths } = await this.workspaceService.resolve();
    const credentialsPath = require("path").join(paths.cache, "credentials.json");
    try {
      const data = await require("fs/promises").readFile(credentialsPath, "utf-8");
   ...
   * フォールバック: 一覧取得
   */
  async listCredentialsFallback() {
    const { paths } = await this.workspaceService.resolve();
    const credentialsPath = require("path").join(paths.cache, "credentials.json");
    try {
      const data = await require("fs/promises").readFile(credentialsPath, "utf-8");
      const credentials = JSON.parse(data);
      return Object.entries(credentials).map(([id, cred]) => ({
        id,
...
// app/src/main/services/PdfJobService.ts
init_cjs_shims();
var import_events = require("events");
var import_uuid2 = require("uuid");
var PdfJobService = class _PdfJobService extends import_events.EventEmitter {
  static instance;
  workspaceService;
  dbService;
  ...
  static getInstance() {
    if (!_PdfJobService.instance) {
      _PdfJobService.instance = new _PdfJobService();
    }
    return _PdfJobService.instance;
  }
  /**
   * サービス初期化
   */
  async initialize() {
    ...
    console.log("\u{1F3ED} Initializing PdfJobService...");
    try {
   ...
   */
  async shutdown() {
    console.log("\u{1F6D1} Shutting down PdfJobService...");
    for (const [jobId, controller] of this.runningJobs) {
      controller.abort();
      await this.updateJobStatus(jobId, "canceled", 0, void 0, "Service shutdown");
    }
  ...
  async enqueue(input) {
    ...
    try {
      ...
      console.log(`\u{1F4E5} Job enqueued: ${jobId} (${input.type})`);
  ...
  async getStatus(jobId) {
    ...
    if (job.status === "running" && this.runningJobs.has(jobId)) {
      ...
      `);
      const startLog = logStmt.get(jobId);
    ...
    return {
      ...
      result: job.result ? JSON.parse(job.result) : void 0,
      error: job.error,
  ...
  async cancel(jobId) {
    ...
    await this.updateJobStatus(jobId, "canceled", 0, void 0, "Canceled by user request");
    this.dbService.addAuditLog({
      action: "job_canceled",
      entity: "job",
      entity_id: jobId,
    ...
    console.log(`\u{1F6AB} Job canceled: ${jobId}`);
  }
  /**
   * 全ジョブの統計を取得
   */
  ...
  async processQueuedJobs() {
    ...
    const stmt = db.prepare(`
      ...
      LIMIT ?
    `);
    const jobs = stmt.all(maxConcurrent - this.runningJobs.size);
    for (const job of jobs) {
      await this.executeJob(job.id, job.type, JSON.parse(job.payload));
    }
  }
  /**
   * ジョブを実行
   */
  async executeJob(jobId, type, payload) {
    ...
    console.log(`\u{1F680} Job started: ${jobId} (${type})`);
    try {
      let result;
      switch (type) {
        case "pdf_generation":
      ...
      await this.updateJobStatus(jobId, "failed", 0, void 0, errorMessage);
      this.dbService.addAuditLog({
        action: "job_failed",
        entity: "job",
        entity_id: jobId,
   ...
   * PDF生成ジョブを実行
   */
  async executePdfGeneration(jobId, payload, signal) {
    try {
   ...
   * Markdownコンパイルジョブを実行
   */
  async executeMarkdownCompile(jobId, payload, signal) {
    try {
   ...
   * クリーンアップジョブを実行
   */
  async executeCleanup(jobId, payload, signal) {
    try {
   ...
   * ジョブ状態を更新
   */
  async updateJobStatus(jobId, status, progress, result, error) {
    const db = this.dbService["getDb"]();
    const now = (/* @__PURE__ */ new Date()).toISOString();
    const stmt = db.prepare(`
      UPDATE jobs 
  ...
  async cleanupStaleJobs() {
    const db = this.dbService["getDb"]();
    const staleThreshold = new Date(Date.now() - 24 * 60 * 60 * 1e3).toISOString();
    const stmt = db.prepare(`
      UPDATE jobs 
      SET status = 'failed', error = 'Job timed out (stale cleanup)', updated_at = ?
      WHERE status = 'running' AND updated_at < ?
    `);
    const result = stmt.run((/* @__PURE__ */ new Date()).toISOString(), staleThreshold);
    if (result.changes > 0) {
      console.log(`\u{1F9F9} Cleaned up ${result.changes} stale jobs`);
    }
  }
};

// app/src/main/services/SyncService.ts
init_cjs_shims();
var import_events2 = require("events");
var SyncService = class _SyncService extends import_events2.EventEmitter {
  static instance;
  workspaceService;
  dbService;
   ...
   */
  async shutdown() {
    console.log("\u{1F6D1} Shutting down SyncService...");
  ...
  async synchronize() {
    ...
    console.log("\u{1F504} Starting manual synchronization...");
    try {
      this.setSyncStatus("connecting");
      const config2 = await this.getSyncConfig();
      if (!config2.enabled || !config2.endpoint) {
        throw new Error("Sync not configured or disabled");
      }
      const online = await this.testConnection(config2);
      if (!online) {
        throw new Error("Unable to connect to sync server");
      }
      this.setSyncStatus("syncing");
      const uploadResult = await this.uploadChanges(config2);
      const downloadResult = await this.downloadChanges(config2);
   ...
   */
  async recordChange(entity, entity_id, operation, data) {
    try {
      const db = this.dbService["getDb"]();
      ...
      const config2 = await this.getSyncConfig();
      if (config2.enabled && this.syncStatus === "online") {
        this.emit("change:recorded", changeEntry);
      }
      console.log(`\u{1F4DD} Change recorded: ${entity}:${entity_id} (${operation})`);
    } catch (error) {
    ...
    try {
      if (updates.enabled !== void 0) {
        if (updates.enabled && !this.syncInterval) {
          const config2 = await this.getSyncConfig();
          this.startAutoSync({ ...config2, ...updates });
  ...
  startAutoSync(config2) {
    ...
    }, config2.syncIntervalMs);
    console.log(`\u23F0 Auto sync started (interval: ${config2.syncIntervalMs}ms)`);
  }
  /**
   * 自動同期停止
   */
  ...
  setSyncStatus(status) {
    if (this.syncStatus !== status) {
      ...
      console.log(`\u{1F504} Sync status: ${status}`);
    }
  }
  /**
   * デバイスID生成・確保
  ...
  async ensureDeviceId() {
    try {
      ...
      try {
        const deviceData2 = JSON.parse(
          await require("fs/promises").readFile(devicePath, "utf-8")
        );
        if (deviceData2.id && typeof deviceData2.id === "string") {
          this.deviceId = deviceData2.id;
          return;
        }
      } catch (error) {
      }
      ...
      console.log(`\u{1F4F1} Device ID created: ${this.deviceId}`);
    } catch (error) {
  ...
  async initializeVectorClock() {
    try {
      ...
      const stmt = db.prepare(`
        ...
        ORDER BY created_at DESC LIMIT 1
      `);
      const result = stmt.get(this.deviceId);
   ...
   * 次のバージョン番号取得
   */
  async getNextVersion(entity, entity_id) {
    try {
      const db = this.dbService["getDb"]();
      const stmt = db.prepare(`
      ...
      `);
      const result = stmt.get(entity, entity_id);
   ...
   * 変更のアップロード（スタブ実装）
   */
  async uploadChanges(config2) {
    try {
      console.log("\u{1F4E4} Uploading changes...");
      const db = this.dbService["getDb"]();
      const stmt = db.prepare(`
        ...
        LIMIT ?
      `);
      const changes = stmt.all(config2.batchSize);
      console.log(`\u{1F4E4} Would upload ${changes.length} changes`);
   ...
   * 変更のダウンロード（スタブ実装）
   */
  async downloadChanges(config2) {
    try {
      console.log("\u{1F4E5} Downloading changes...");
      const count = 0;
      console.log(`\u{1F4E5} Downloaded ${count} changes`);
      return { count };
    } catch (error) {
   ...
   * 同期完了記録
   */
  async recordSyncCompletion(result) {
    try {
      this.dbService.addAuditLog({
...
// app/src/main/services/SearchService.ts
init_cjs_shims();
var import_events3 = require("events");
var SearchService = class _SearchService extends import_events3.EventEmitter {
  static instance;
  workspaceService;
  dbService;
  secureStorageService;
  ...
  async initialize() {
    ...
    console.log("\u{1F50D} Initializing SearchService...");
    try {
  ...
  async search(query) {
    ...
    console.log(`\u{1F50D} Searching: "${query.query}" (${query.type})`);
    try {
      let results = [];
      switch (query.type) {
        case "fulltext":
      ...
      const took_ms = performance.now() - startTime2;
      const searchResult = {
   ...
   * 埋め込み生成
   */
  async generateEmbedding(entity, entity_id, content, force = false) {
    try {
      const config2 = await this.getEmbeddingConfig();
      if (!config2.enabled) {
        return null;
      }
      if (!force) {
        const existing = await this.getExistingEmbedding(entity, entity_id, config2.model);
        if (existing) {
          return existing;
        }
      }
      console.log(`\u{1F9EE} Generating embedding for ${entity}:${entity_id}`);
      let vector;
      if (config2.provider === "openai") {
        vector = await this.generateOpenAIEmbedding(content, config2);
      } else {
        vector = await this.generateLocalEmbedding(content, config2);
      }
      const result = {
        entity_type: entity,
        entity_id,
  ...
  async rebuildIndexes() {
    console.log("\u{1F504} Rebuilding search indexes...");
    const result = {
      fts_rebuilt: false,
  ...
  async getSearchStats() {
    try {
      ...
      `);
      const indexedResult = indexedStmt.get();
      const embeddingsStmt = db.prepare("SELECT COUNT(*) as count FROM embeddings");
      const embeddingsResult = embeddingsStmt.get();
      const ftsIndexes = [
        "meetings_fts",
        "documents_fts",
      ...
      const lastUpdateStmt = db.prepare(`
        ...
        ORDER BY created_at DESC LIMIT 1
      `);
      const lastUpdateResult = lastUpdateStmt.get();
      return {
   ...
   * エンティティ内検索
   */
  async searchInEntity(db, entity, query, filters) {
    let searchSQL;
    const params = [query];
    switch (entity) {
      case "meetings":
  ...
  async performVectorSearch(query) {
    try {
      ...
      console.log("\u{1F50D} Vector search not yet implemented");
      return [];
    } catch (error) {
   ...
   * OpenAI埋め込み生成（スタブ実装）
   */
  async generateOpenAIEmbedding(content, config2) {
    try {
      const vector = Array.from({ length: config2.dimensions }, () => Math.random() - 0.5);
   ...
   * ローカル埋め込み生成（スタブ実装）
   */
  async generateLocalEmbedding(content, config2) {
    try {
      const vector = Array.from({ length: config2.dimensions }, (_, i) => {
        const hash = this.simpleHash(content + i);
        return (hash % 2e3 - 1e3) / 1e3;
      });
      return vector;
    } catch (error) {
   ...
   * 既存埋め込み取得
   */
  async getExistingEmbedding(entity, entity_id, model) {
    try {
      const db = this.dbService["getDb"]();
      const stmt = db.prepare(`
        ...
        WHERE entity_type = ? AND entity_id = ? AND model = ?
      `);
      const row = stmt.get(entity, entity_id, model);
      if (!row) {
        return null;
      }
  ...
  async checkEmbeddingCoverage() {
    try {
      const db = this.dbService["getDb"]();
      const entities = ["meetings", "documents", "document_versions", "transcript_segments", "tasks"];
      for (const entity of entities) {
        ...
        const embeddedStmt = db.prepare(`
          ...
          WHERE entity_type = ?
        `);
        const embeddedResult = embeddedStmt.get(entity);
        const coverage = totalResult.total > 0 ? Math.round(embeddedResult.embedded / totalResult.total * 100) : 100;
        console.log(`\u{1F4CA} ${entity} embedding coverage: ${embeddedResult.embedded}/${totalResult.total} (${coverage}%)`);
      }
    } catch (error) {
...
var isHttps = /https:?/;
var supportedProtocols = platform_default.protocols.map((protocol2) => {
  return protocol2 + ":";
});
...
var http_default = isHttpAdapterSupported && function httpAdapter(config2) {
  return wrapAsync(async function dispatchHttpRequest(resolve3, reject, onDone) {
    ...
    const emitter = new import_events4.EventEmitter();
    const onFinished = () => {
...
// app/src/main/services/ChunkingService.ts
init_cjs_shims();
var ChunkingService = class _ChunkingService {
  ...
  defaultConfig = {
    maxChunkSize: 5e3,
    // 本番用: 5000文字（lost in the middle対策）
    overlapSize: 100,
    // 本番用: 100文字
  ...
  static getInstance() {
    if (!_ChunkingService.instance) {
      _ChunkingService.instance = new _ChunkingService();
    }
    return _ChunkingService.instance;
  }
  /**
   * VTTテキストを分割処理
   */
  async chunkText(text, config2 = {}) {
    const startTime2 = Date.now();
    const finalConfig = { ...this.defaultConfig, ...config2 };
    console.log("\u{1F504} Starting text chunking...", {
   ...
   * 実際の分割処理
   */
  async performChunking(text, config2) {
    const chunks = [];
    const { maxChunkSize, overlapSize, splitOnSentence, preserveSpeakers } = config2;
  ...
  mergeChunks(processedChunks, originalChunks) {
    console.log("\u{1F504} Starting chunk merging...");
    if (processedChunks.length === 0) {
      return "";
    }
    const sortedProcessed = processedChunks.sort(
  ...
  displayChunkingResult(result) {
    console.log("\n\u{1F50D} Chunking Result Analysis:");
...
var AIProcessingService = class _AIProcessingService {
  ...
  static getInstance() {
    if (!_AIProcessingService.instance) {
      _AIProcessingService.instance = new _AIProcessingService();
    }
    return _AIProcessingService.instance;
  }
  /**
   * テキスト処理のメインAPI（分割処理対応）
   */
  async processText(inputText, templateId, customPrompt, options) {
    const startTime2 = Date.now();
    const processingOptions = { ...this.defaultOptions, ...options };
    const warnings = [];
    try {
      console.log(`\u{1F916} AI\u51E6\u7406\u958B\u59CB - Provider: ${processingOptions.provider}`);
      console.log(`\u{1F4CA} \u5165\u529B\u30C6\u30AD\u30B9\u30C8: ${inputText.length}\u6587\u5B57`);
      ...
      if (processingOptions.provider !== "offline") {
        console.log("\u{1F504} \u30AA\u30D5\u30E9\u30A4\u30F3\u51E6\u7406\u306B\u30D5\u30A9\u30FC\u30EB\u30D0\u30C3\u30AF...");
  ...
  async processWithChunking(inputText, prompt, options) {
    const chunkingConfig = {
      maxChunkSize: options.provider === "mock" ? 300 : 5e3,
      // モック用300文字、本番用5000文字
    ...
    console.log("\u{1F504} Step 1: \u30C6\u30AD\u30B9\u30C8\u5206\u5272\u4E2D...");
    ...
    console.log(`\u{1F504} Step 2: ${chunkingResult.totalChunks}\u500B\u306E\u30C1\u30E3\u30F3\u30AF\u3092LLM\u51E6\u7406\u4E2D...`);
    ...
    for (let i = 0; i < chunkingResult.chunks.length; i++) {
      ...
      options.onProgress?.(progress);
      try {
        console.log(`\u{1F504} \u30C1\u30E3\u30F3\u30AF ${i + 1}/${chunkingResult.totalChunks} \u3092\u51E6\u7406\u4E2D...`);
    ...
    console.log("\u{1F504} Step 3: \u7D50\u679C\u7D71\u5408\u4E2D...");
   ...
   * 通常の単一処理
   */
  async processSingle(inputText, prompt, options) {
   ...
   * 残り時間の推定計算
   */
  calculateEstimatedTime(processed, total, startTime2) {
    if (processed === 0) return 0;
    const elapsed = Date.now() - startTime2;
    const averageTime = elapsed / processed;
    const remaining = total - processed;
    return Math.round(remaining * averageTime / 1e3);
  }
  /**
   * Gemini APIを使用した処理
   */
  async processWithGemini(text, prompt, options) {
    ...
    if (!apiKey && !isProduction) {
      ...
      console.log("\u{1F511} [\u958B\u767A\u74B0\u5883] Using Gemini API key from environment variables");
    }
    if (!apiKey) {
      const message = isProduction ? "Gemini API key not configured. Please set your API key in Settings > API Configuration." : "Gemini API key not found in SecureStorage or environment variables";
...
${text}` }]
   ...
   * OpenAI APIを使用した処理
   */
  async processWithOpenAI(text, prompt, options) {
    let apiKey = await this.secureStorage.getCredential("openai_api_key");
    const isProduction = process.env.NODE_ENV === "production" || process.env.VITE_FORCE_USER_API_KEYS === "true";
    if (!apiKey && !isProduction) {
      apiKey = process.env.OPENAI_API_KEY || process.env.VITE_OPENAI_API_KEY;
      console.log("\u{1F511} [\u958B\u767A\u74B0\u5883] Using OpenAI API key from environment variables");
    }
    if (!apiKey) {
      const message = isProduction ? "OpenAI API key not configured. Please set your API key in Settings > API Configuration." : "OpenAI API key not found in SecureStorage or environment variables";
      throw new Error(message);
    }
    const request = {
   ...
   * モック処理（デモ・テスト用）
   */
  async processWithMock(text, prompt, options) {
    await new Promise((resolve3) => setTimeout(resolve3, 1e3 + Math.random() * 2e3));
   ...
   * オフライン処理（ルールベース）
   */
  async processOffline(text, prompt, options) {
    console.log("\u{1F527} \u30AA\u30D5\u30E9\u30A4\u30F3\u51E6\u7406\u3067\u30C6\u30AD\u30B9\u30C8\u3092\u4FEE\u6B63\u4E2D...");
   ...
   * 議事録形式への変換
   */
  formatAsMinutes(text) {
    const lines = text.split("\n").filter((line) => line.trim());
    let formatted = "";
    formatted += "# \u4F1A\u8B70\u8B70\u4E8B\u9332\n\n";
    formatted += `**\u4F5C\u6210\u65E5\u6642**: ${(/* @__PURE__ */ new Date()).toLocaleDateString("ja-JP")}
`;
...
`;
   ...
   * モック結果生成
   */
  generateMockResult(originalText) {
    const mockHeader = `# \u4F1A\u8B70\u8B70\u4E8B\u9332\uFF08\u30E2\u30C3\u30AF\u51E6\u7406\uFF09

**\u65E5\u6642**: ${(/* @__PURE__ */ new Date()).toLocaleDateString("ja-JP")}
**\u51E6\u7406**: AI\u6A21\u64EC\u51E6\u7406
...
`;
  ...
  async getPromptTemplate(templateId) {
    try {
      const { paths } = await this.workspaceService.resolve();
      console.log("\u{1F50D} Workspace paths:", JSON.stringify(paths, null, 2));
      if (!paths.templates) {
        console.error("\u274C templates path is undefined:", paths);
        console.log("\u{1F504} Using fallback template path...");
        const { root } = await this.workspaceService.resolve();
        const fallbackTemplatePath = path7.join(root, "templates", `${templateId}.json`);
        console.log("\u{1F4C1} Fallback template path:", fallbackTemplatePath);
        const content2 = await fs6.readFile(fallbackTemplatePath, "utf-8");
        return JSON.parse(content2);
      }
      const templatePath = path7.join(paths.templates, `${templateId}.json`);
      console.log("\u{1F4C1} Template path:", templatePath);
...
\u4F59\u8A08\u306A\u8AAC\u660E\u306F\u542B\u3081\u305A\u3001\u4FEE\u6B63\u3055\u308C\u305F\u30C6\u30AD\u30B9\u30C8\u306E\u307F\u3092\u51FA\u529B\u3057\u3066\u304F\u3060\u3055\u3044\u3002`;
   ...
   * テキスト再修正
   */
  async reviseText(originalText, revisionNotes, options) {
...
var SECURITY_CONFIG = {
  CSP: {
    ...
    SCRIPT: process.env.NODE_ENV === "development" ? "script-src 'self' 'unsafe-inline' 'unsafe-eval' http://localhost:5173" : "script-src 'self'",
    // 本番環境では厳格化
...
var SecurityPolicy = class _SecurityPolicy {
  ...
  static getInstance() {
    if (!_SecurityPolicy.instance) {
      _SecurityPolicy.instance = new _SecurityPolicy();
    }
    return _SecurityPolicy.instance;
  }
  /**
   * app.ready前に実行すべきプロトコル設定
   */
  setupProtocolsBeforeReady() {
    if (this.protocolsRegistered) {
      return;
    }
    console.log("\u{1F6E1}\uFE0F Registering security protocols before app ready...");
    try {
  ...
  async initialize() {
    ...
    console.log("\u{1F512} Initializing security policies...");
    try {
  ...
  setupContentSecurityPolicy() {
    ...
    console.log("\u{1F6E1}\uFE0F Content Security Policy configured");
  }
  /**
   * プロトコルハンドラー設定（app.ready後の処理）
   */
  setupProtocolHandlersAfterReady() {
    const dangerousProtocols = ["file:", "ftp:", "chrome:", "chrome-extension:"];
    import_electron5.session.defaultSession.webRequest.onBeforeRequest({ urls: ["*://*/*"] }, (details, callback) => {
      const url2 = new URL(details.url);
      if (dangerousProtocols.some((proto) => details.url.startsWith(proto))) {
        console.warn("\u{1F6AB} Blocked dangerous protocol:", details.url);
        callback({ cancel: true });
        return;
      }
      if (!SECURITY_CONFIG.ALLOWED_PROTOCOLS.some((proto) => details.url.startsWith(proto))) {
        console.warn("\u{1F6AB} Blocked non-allowed protocol:", details.url);
        callback({ cancel: true });
        return;
      }
      callback({ cancel: false });
    });
    console.log("\u{1F517} Protocol handlers configured");
  }
  /**
   * セッションセキュリティ設定
   */
  setupSessionSecurity() {
    const defaultSession = import_electron5.session.defaultSession;
    ...
    defaultSession.setPermissionRequestHandler((webContents, permission, callback) => {
      console.log("\u{1F510} Permission request:", permission);
      const allowedPermissions = ["clipboard-read", "clipboard-write"];
      callback(allowedPermissions.includes(permission));
    });
    const preloadPath = path8.join(__dirname, "../preload.cjs");
    try {
    ...
    console.log("\u{1F510} Session security configured");
  }
  /**
   * ウィンドウセキュリティ設定
   */
  setupWindowSecurity() {
    const originalCreateWindow = import_electron5.BrowserWindow.prototype.constructor;
    const secureWindowDefaults = {
      webPreferences: {
        ...
        disableBlinkFeatures: void 0,
        preload: path8.join(__dirname, "../preload.cjs")
      },
      show: false
      // 初期状態では非表示
    };
    console.log("\u{1FA9F} Window security defaults configured");
  }
  /**
   * ファイルシステムアクセス検証
   */
  validateFilePath(filePath) {
    try {
      ...
      if (normalizedPath.includes("..") || normalizedPath.includes("~")) {
        console.warn("\u{1F6AB} Path traversal attempt detected:", filePath);
        return false;
      }
      ...
      if (restrictedPaths.some(
        (restricted) => lowerPath.startsWith(restricted.toLowerCase())
      )) {
        console.warn("\u{1F6AB} Access to restricted path denied:", filePath);
        return false;
      }
      return true;
    } catch (error) {
  ...
  validateUrl(url2) {
    try {
      ...
      if (!["https:", "http:"].includes(parsedUrl.protocol)) {
        console.warn("\u{1F6AB} Invalid protocol:", parsedUrl.protocol);
        return false;
      }
      ...
      const isAllowed = allowedDomains.some(
        (domain) => parsedUrl.hostname.endsWith(domain) || parsedUrl.hostname === domain
      );
      if (!isAllowed) {
        console.warn("\u{1F6AB} Domain not allowed:", parsedUrl.hostname);
        return false;
      }
      return true;
    } catch (error) {
  ...
  logSecurityEvent(event, details) {
    ...
    console.log("\u{1F50D} Security Event:", JSON.stringify(logEntry));
  }
  /**
   * セキュリティヘルスチェック
   */
  healthCheck() {
    const results = {
      ...
      session: !!import_electron5.session.defaultSession,
      protocols: import_electron5.protocol.isProtocolHandled("https"),
      overall: false
    };
...
function setupFileHandler() {
  import_electron6.ipcMain.handle("file:select", async (event, options) => {
    try {
      const result = await import_electron6.dialog.showOpenDialog(import_electron6.BrowserWindow.fromWebContents(event.sender), {
        properties: ["openFile"],
        filters: [
  ...
  import_electron6.ipcMain.handle("file:read", async (event, filePath) => {
    try {
      if (!filePath) {
      ...
      const content = await fs7.readFile(normalizedPath, "utf-8");
      const stats = await fs7.stat(normalizedPath);
      return {
        content,
        size: stats.size,
        lastModified: stats.mtime,
        filename: path9.basename(normalizedPath)
      };
    } catch (error) {
  ...
  import_electron6.ipcMain.handle("file:write", async (event, filePath, content) => {
    try {
      ...
      const directory = path9.dirname(normalizedPath);
      await fs7.mkdir(directory, { recursive: true });
      await fs7.writeFile(normalizedPath, content, "utf-8");
      return {
        success: true,
  ...
  import_electron6.ipcMain.handle("file:save-dialog", async (event, options) => {
    try {
      const result = await import_electron6.dialog.showSaveDialog(import_electron6.BrowserWindow.fromWebContents(event.sender), {
        filters: [
  ...
  import_electron6.ipcMain.handle("file:exists", async (event, filePath) => {
    try {
      const normalizedPath = path9.normalize(filePath);
      await fs7.access(normalizedPath);
      return true;
    } catch {
      return false;
    }
  });
  import_electron6.ipcMain.handle("file:copy", async (event, srcPath, destPath) => {
    try {
      if (!srcPath || !destPath) {
        throw new Error("Source and destination paths are required");
      }
      const normalizedSrc = path9.normalize(srcPath);
      const normalizedDest = path9.normalize(destPath);
      if (normalizedSrc.includes("..") || normalizedDest.includes("..")) {
        throw new Error("Invalid file path");
      }
      const destDirectory = path9.dirname(normalizedDest);
      await fs7.mkdir(destDirectory, { recursive: true });
      await fs7.copyFile(normalizedSrc, normalizedDest);
      const stats = await fs7.stat(normalizedDest);
      return {
        success: true,
        data: {
  ...
  import_electron6.ipcMain.handle("file:copy-to-downloads", async (event, srcPath, filename) => {
    try {
      if (!srcPath || !filename) {
      ...
      const downloadsPath = path9.join(os.homedir(), "Downloads", filename);
      await new Promise((resolve3) => setTimeout(resolve3, 500));
      try {
        await fs7.access(downloadsPath);
        await fs7.unlink(downloadsPath);
        console.log("\u{1F5D1}\uFE0F Existing file removed:", downloadsPath);
      } catch (error) {
      }
      let retryCount = 0;
      const maxRetries = 3;
      while (retryCount < maxRetries) {
        try {
          await fs7.copyFile(normalizedSrc, downloadsPath);
      ...
      const stats = await fs7.stat(downloadsPath);
      return {
        success: true,
        data: {
  ...
  console.log("\u{1F4C2} File handler IPC registered");
}

// app/src/main/ipc/storage-handler.ts
init_cjs_shims();
var import_electron7 = require("electron");
var fs8 = __toESM(require("fs/promises"), 1);
var path10 = __toESM(require("path"), 1);
var import_electron8 = require("electron");
var getConfigPath = () => path10.join(import_electron8.app.getPath("userData"), "config");
var getStorageFilePath = (filename) => path10.join(getConfigPath(), filename);
function setupStorageHandler() {
  import_electron7.ipcMain.handle("storage:get", async (event, key) => {
    try {
  ...
  import_electron7.ipcMain.handle("storage:set", async (event, key, value) => {
    try {
      const configPath = getStorageFilePath("app-config.json");
  ...
  import_electron7.ipcMain.handle("storage:delete", async (event, key) => {
    try {
      const configPath = getStorageFilePath("app-config.json");
      try {
        const content = await fs8.readFile(configPath, "utf-8");
  ...
  import_electron7.ipcMain.handle("storage:get-all", async (event) => {
    try {
      const configPath = getStorageFilePath("app-config.json");
      try {
        const content = await fs8.readFile(configPath, "utf-8");
  ...
  import_electron7.ipcMain.handle("storage:clear", async (event) => {
    try {
      const configPath = getStorageFilePath("app-config.json");
      try {
  ...
  console.log("\u{1F4BE} Storage handler IPC registered");
}

// app/src/main/ipc/workspace-handler.ts
init_cjs_shims();
...
var FilePathSchema = external_exports.string().min(1).refine(
  (path12) => !path12.includes("..") && !path12.includes("~"),
...
var FileSelectOptionsSchema = external_exports.object({
  title: external_exports.string().optional(),
  defaultPath: FilePathSchema.optional(),
  buttonLabel: external_exports.string().optional(),
  filters: external_exports.array(external_exports.object({
    name: external_exports.string(),
    extensions: external_exports.array(external_exports.string())
  })).optional(),
  properties: external_exports.array(external_exports.enum([
    "openFile",
    "openDirectory",
    "multiSelections",
    "showHiddenFiles",
...
var FileSelectResultSchema = external_exports.object({
  canceled: external_exports.boolean(),
  filePaths: external_exports.array(FilePathSchema)
});
var SaveDialogOptionsSchema = external_exports.object({
  title: external_exports.string().optional(),
  defaultPath: FilePathSchema.optional(),
  buttonLabel: external_exports.string().optional(),
  filters: external_exports.array(external_exports.object({
  ...
  nameFieldLabel: external_exports.string().optional(),
  showsTagField: external_exports.boolean().optional()
});
var SaveDialogResultSchema = external_exports.object({
  canceled: external_exports.boolean(),
  filePath: FilePathSchema.optional()
});
var StorageKeySchema = external_exports.string().min(1).max(256).refine(
  (key) => /^[a-zA-Z0-9_\-\.]+$/.test(key),
  { message: "Storage key contains invalid characters" }
);
var StorageValueSchema = external_exports.union([
...
var BackendRequestOptionsSchema = external_exports.object({
  ...
  timeout: external_exports.number().positive().max(3e5).optional()
  // 最大5分
});
var WorkspaceInfoSchema = external_exports.object({
  root: FilePathSchema,
  paths: external_exports.record(FilePathSchema),
  currentPath: external_exports.string(),
  isPortable: external_exports.boolean(),
  canWrite: external_exports.boolean(),
  diskUsage: external_exports.object({
...
var WorkspaceConfigSchema2 = external_exports.object({
...
var AppLocalConfigSchema2 = external_exports.object({
  pdf: external_exports.object({
    pageSize: external_exports.enum(["A4", "Letter"]),
    marginMm: external_exports.number().positive().max(50)
    // 最大50mm
  }),
  markdown: external_exports.object({
    theme: external_exports.enum(["default", "corporate"]),
    toc: external_exports.boolean(),
    latex: external_exports.enum(["katex", "tectonic"])
  }),
  mermaid: external_exports.object({
    themePath: external_exports.string()
  })
});
var AppLocalConfigUpdateSchema = AppLocalConfigSchema2.deepPartial();
var MarkdownCompileOptionsSchema = external_exports.object({
  latex: external_exports.enum(["katex", "tectonic"]).optional(),
  theme: external_exports.enum(["default", "corporate"]).optional(),
  toc: external_exports.boolean().optional(),
  pageSize: external_exports.enum(["A4", "Letter"]).optional(),
  marginMm: external_exports.number().positive().max(50).optional()
});
var MarkdownCompileInputSchema = external_exports.object({
  mdPath: FilePathSchema.optional(),
  mdContent: external_exports.string().max(1e7).optional(),
  // 最大10MB
...
var MarkdownCompileResultSchema = external_exports.object({
...
var MarkdownTestResultSchema = external_exports.object({
...
var JobCreateInputSchema = external_exports.object({
  type: JobTypeSchema,
  payload: external_exports.record(external_exports.any()),
  priority: external_exports.enum(["low", "normal", "high"]).optional().default("normal"),
  timeout: external_exports.number().positive().max(18e5).optional()
  // 最大30分
});
var JobCreateResultSchema = external_exports.object({
  id: UuidSchema
});
var JobStatusResultSchema = external_exports.object({
...
var DatabaseHealthCheckSchema = external_exports.object({
...
var DatabaseStatsSchema = external_exports.object({
  users: external_exports.number().int().nonnegative(),
  meetings: external_exports.number().int().nonnegative(),
  documents: external_exports.number().int().nonnegative(),
  tasks: external_exports.number().int().nonnegative(),
  attachments: external_exports.number().int().nonnegative(),
  jobs: external_exports.record(external_exports.number().int().nonnegative()),
  diskUsageMB: external_exports.number().positive().optional(),
  lastVacuum: TimestampSchema.optional()
});
var MermaidCacheStatsSchema = external_exports.object({
  fileCount: external_exports.number().int().nonnegative(),
  totalSizeMB: external_exports.number().nonnegative(),
  oldestFile: external_exports.date().nullable(),
  newestFile: external_exports.date().nullable(),
  hitRate: external_exports.number().min(0).max(100).optional()
});
var MermaidClearResultSchema = external_exports.object({
  success: external_exports.boolean(),
  filesDeleted: external_exports.number().int().nonnegative().optional(),
  spaceSavedMB: external_exports.number().nonnegative().optional()
});
var CredentialTypeSchema = external_exports.enum([
  "supabase_key",
  "api_token",
  "oauth_token",
  "user_credential"
]);
var SecureCredentialInputSchema = external_exports.object({
...
var SecureCredentialListSchema = external_exports.array(external_exports.object({
  id: external_exports.string(),
  type: external_exports.string(),
  service: external_exports.string(),
  account: external_exports.string(),
  hasValue: external_exports.boolean(),
  metadata: external_exports.record(external_exports.any()).optional()
}));
var ApiKeyConfigSchema2 = external_exports.object({
  supabase: external_exports.object({
    url: external_exports.string().url().optional(),
    anonKey: external_exports.string().optional(),
    serviceRoleKey: external_exports.string().optional()
...
var SecureStorageHealthSchema = external_exports.object({
  ok: external_exports.boolean(),
  keytar: external_exports.boolean(),
  fallback: external_exports.boolean(),
  message: external_exports.string()
});
var AppVersionSchema = external_exports.string().regex(
  /^\d+\.\d+\.\d+(-[a-zA-Z0-9\-\.]+)?$/,
  { message: "Invalid version format" }
);
var AppPathNameSchema = external_exports.enum([
  "home",
  "appData",
  "userData",
  "cache",
...
var WorkspaceSelectDirectoryResultSchema = external_exports.object({
  success: external_exports.boolean(),
  path: external_exports.string().optional(),
  error: external_exports.string().optional()
});
var WorkspaceSwitchResultSchema = external_exports.object({
  success: external_exports.boolean(),
  error: external_exports.string().optional()
});
var WorkspaceInitializeResultSchema = external_exports.object({
  success: external_exports.boolean(),
  error: external_exports.string().optional()
});
var WorkspaceBackupResultSchema = external_exports.object({
  success: external_exports.boolean(),
  backupPath: external_exports.string().optional(),
  error: external_exports.string().optional()
});
var IPCErrorSchema = external_exports.object({
  name: external_exports.string(),
  message: external_exports.string(),
  code: external_exports.string().optional(),
  details: external_exports.record(external_exports.any()).optional(),
...
function setupWorkspaceHandler() {
  const workspaceService2 = WorkspaceService.getInstance();
  import_electron9.ipcMain.handle("workspace:getInfo", async () => {
    try {
      const workspace = await workspaceService2.resolve();
      const workspacePath = workspace.root;
      const stats = await fs9.stat(workspacePath);
      const diskUsage = await calculateDiskUsage(workspacePath);
      const isPortable = workspacePath.includes("portable") || process.env.GIJIROKU_WORKSPACE_PATH !== void 0;
      const canWrite = await checkWritePermission(workspacePath);
      const version = await getWorkspaceVersion(workspacePath);
      const info = {
        root: workspacePath,
        paths: workspace.paths || {
          attachments: path11.join(workspacePath, "attachments"),
          exports: path11.join(workspacePath, "exports"),
          templates: path11.join(workspacePath, "templates"),
          data: path11.join(workspacePath, "data")
        },
  ...
  import_electron9.ipcMain.handle("workspace:selectDirectory", async (event) => {
    try {
  ...
  import_electron9.ipcMain.handle("workspace:switchTo", async (event, newPath) => {
    try {
      const resolvedPath = path11.resolve(newPath);
      try {
      ...
      await workspaceService2.initIfNeeded();
  ...
  import_electron9.ipcMain.handle("workspace:initialize", async () => {
    try {
      await workspaceService2.initIfNeeded();
  ...
  import_electron9.ipcMain.handle("workspace:backup", async () => {
    try {
      const workspace = await workspaceService2.resolve();
      const workspacePath = workspace.root;
      const timestamp = (/* @__PURE__ */ new Date()).toISOString().replace(/[:.]/g, "-");
      const backupName = `workspace-backup-${timestamp}`;
      const result = await import_electron9.dialog.showSaveDialog({
        title: "\u30D0\u30C3\u30AF\u30A2\u30C3\u30D7\u5148\u3092\u9078\u629E",
...
async function calculateDiskUsage(dirPath) {
  try {
    const used = await getDirectorySize(dirPath);
    const stats = await fs9.statfs(dirPath);
...
async function getDirectorySize(dirPath) {
  let totalSize = 0;
  try {
    const items = await fs9.readdir(dirPath, { withFileTypes: true });
    for (const item of items) {
      const itemPath = path11.join(dirPath, item.name);
      if (item.isDirectory()) {
        totalSize += await getDirectorySize(itemPath);
      } else if (item.isFile()) {
        const stats = await fs9.stat(itemPath);
        totalSize += stats.size;
      }
    }
  } catch {
...
async function checkWritePermission(dirPath) {
  try {
    const testFile = path11.join(dirPath, ".write-test");
...
async function getWorkspaceVersion(workspacePath) {
  try {
    const configPath = path11.join(workspacePath, "config", "workspace.json");
    const content = await fs9.readFile(configPath, "utf-8");
    const config2 = JSON.parse(content);
    return config2.version || "1.0.0";
  } catch {
    return "1.0.0";
  }
}
async function copyDirectory(src, dest) {
  ...
  const items = await fs9.readdir(src, { withFileTypes: true });
  for (const item of items) {
    const srcPath = path11.join(src, item.name);
    const destPath = path11.join(dest, item.name);
    if (item.isDirectory()) {
...
// app/src/main/index.ts
dotenv.config();
console.log("\u{1F4CB} Environment loaded:", {
  GEMINI_KEY: process.env.GEMINI_API_KEY ? "\u2705 Set" : "\u274C Not set",
  OPENAI_KEY: process.env.OPENAI_API_KEY ? "\u2705 Set" : "\u274C Not set"
});
var startTime = import_perf_hooks.performance.now();
var windowManager = new WindowManager();
var workspaceService = WorkspaceService.getInstance();
var dbService = DbService.getInstance();
var markdownCompiler = MarkdownCompilerService.getInstance();
var mermaidWorker = MermaidRenderWorker.getInstance();
var pdfGenerationService = PdfGenerationService.getInstance();
var secureStorageService = SecureStorageService.getInstance();
var pdfJobService = PdfJobService.getInstance();
var syncService = SyncService.getInstance();
var searchService = SearchService.getInstance();
var aiProcessingService = AIProcessingService.getInstance();
var securityPolicy = SecurityPolicy.getInstance();
console.log("\u{1F512} Pre-initializing security protocols...");
securityPolicy.setupProtocolsBeforeReady();
import_electron10.app.whenReady().then(async () => {
  console.log("\u{1F680} Electron application starting...");
  try {
    console.log("\u{1F512} Completing security policy initialization...");
    await securityPolicy.initialize();
    console.log("\u{1F4C1} Initializing workspace...");
    await workspaceService.initIfNeeded();
    console.log("\u{1F5C4}\uFE0F Initializing database...");
    await dbService.initialize();
    console.log("\u{1F510} Initializing secure storage...");
    await secureStorageService.initialize();
    console.log("\u{1F3ED} Initializing job service...");
    await pdfJobService.initialize();
    console.log("\u{1F504} Initializing sync service...");
    await syncService.initialize();
    console.log("\u{1F50D} Initializing search service...");
    ...
    const totalTime = import_perf_hooks.performance.now() - startTime;
    console.log(`\u2705 Application ready in ${totalTime.toFixed(2)}ms`);
  } catch (error) {
    console.error("\u274C Application startup failed:", error);
    import_electron10.app.quit();
  }
  import_electron10.app.on("activate", async () => {
    if (import_electron10.BrowserWindow.getAllWindows().length === 0) {
      await windowManager.createMainWindow();
    }
  });
});
import_electron10.app.on("window-all-closed", async () => {
  console.log("\u{1FA9F} All windows closed, cleaning up...");
  await cleanupServices();
  if (process.platform !== "darwin") {
    import_electron10.app.quit();
  }
});
import_electron10.app.on("before-quit", async () => {
  console.log("\u{1F6D1} Application shutting down...");
  await cleanupServices();
});
...
import_electron10.app.on("web-contents-created", (event, contents) => {
  contents.setWindowOpenHandler(({ url: url2 }) => {
    console.warn("Blocked external navigation:", url2);
    return { action: "deny" };
  });
});
function setupDatabaseHandlers() {
  import_electron10.ipcMain.handle("database:health-check", () => {
    try {
  ...
  import_electron10.ipcMain.handle("database:get-stats", () => {
    try {
...
function setupJobsHandlers() {
  import_electron10.ipcMain.handle("jobs:enqueue", async (event, job) => {
    try {
  ...
  import_electron10.ipcMain.handle("jobs:get-status", async (event, id) => {
    try {
  ...
  import_electron10.ipcMain.handle("jobs:cancel", async (event, id) => {
    try {
  ...
  import_electron10.ipcMain.handle("jobs:get-stats", async (event) => {
    try {
...
function setupMarkdownHandlers() {
  import_electron10.ipcMain.handle("markdown:compile-to-pdf", async (event, input) => {
    try {
      console.log("\u{1F4C4} PDF generation requested:", { inputLength: input?.mdContent?.length });
  ...
  import_electron10.ipcMain.handle("markdown:test-pdf", async (event) => {
    try {
  ...
  import_electron10.ipcMain.handle("mermaid:clear-cache", async (event) => {
    try {
  ...
  import_electron10.ipcMain.handle("mermaid:get-cache-stats", async (event) => {
    try {
...
function setupSecureStorageHandlers() {
  import_electron10.ipcMain.handle("secure:set-credential", async (event, input) => {
    try {
  ...
  import_electron10.ipcMain.handle("secure:get-credential", async (event, id, account) => {
    try {
  ...
  import_electron10.ipcMain.handle("secure:delete-credential", async (event, id, account) => {
    try {
  ...
  import_electron10.ipcMain.handle("secure:list-credentials", async (event) => {
    try {
  ...
  import_electron10.ipcMain.handle("secure:set-api-config", async (event, config2) => {
    try {
      await secureStorageService.setApiConfig(config2);
      return { success: true };
    } catch (error) {
  ...
  import_electron10.ipcMain.handle("secure:get-api-config-status", async (event) => {
    try {
  ...
  import_electron10.ipcMain.handle("secure:health-check", async (event) => {
    try {
...
function setupSyncHandlers() {
  import_electron10.ipcMain.handle("sync:synchronize", async (event) => {
    try {
  ...
  import_electron10.ipcMain.handle("sync:get-status", async (event) => {
    try {
  ...
  import_electron10.ipcMain.handle("sync:update-config", async (event, updates) => {
    try {
  ...
  import_electron10.ipcMain.handle("sync:record-change", async (event, entity, entity_id, operation, data) => {
    try {
...
function setupSearchHandlers() {
  import_electron10.ipcMain.handle("search:search", async (event, query) => {
    try {
  ...
  import_electron10.ipcMain.handle("search:generate-embedding", async (event, entity, entity_id, content, force = false) => {
    try {
  ...
  import_electron10.ipcMain.handle("search:rebuild-indexes", async (event) => {
    try {
  ...
  import_electron10.ipcMain.handle("search:get-stats", async (event) => {
    try {
...
function setupAIHandlers() {
  import_electron10.ipcMain.handle("ai:process-text", async (event, inputText, templateId, customPrompt, options) => {
    try {
  ...
  import_electron10.ipcMain.handle("ai:revise-text", async (event, originalText, revisionNotes, options) => {
    try {
  ...
  import_electron10.ipcMain.handle("ai:get-providers", async (event) => {
    try {
